...
export default {
  ...
  async mounted() {
    ...
    this.headers = [
      ...
      },
      {
        ...
        field_required: true,
        items: this.cities,
        show_in_grid: true, //this.current_city_id == null,
      ...
      },
      {
        ...
        field_required: true,
        items: this.shops,
        show_in_grid: true, //this.current_shop_id == null,
        hide_in_menu_columns: true,
        sortable: true,
      },
      {
  ...
  methods: {
    ...
    getDevices() {
      return new Promise((resolve, reject) => {
        axios.post('/api/device/get_list', {
          start: 0,
          limit: null,
        })
            .then(response => {
    ...
    getItems(data) {
      ...
      axios.get('/api/product/get_list', { params: {
    ...
    getEmptyDevices(editedItem) {
      let selected_macs = editedItem.devices.map(function(m) { return m.mac; });
      let that = this;
      this.headers.forEach(function (value, i) {
        if (value.name === 'devices') {
